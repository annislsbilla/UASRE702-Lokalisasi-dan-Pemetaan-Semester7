...
ScanLines::ScanLines(cv::Mat &image_, unsigned scanType, cv::Point start_, cv::Point end_, unsigned step_x, unsigned step_y)
{
	inRadius = 0;
	outRadius = 0;
	type = scanType;
	image = image_;//image_.copyTo(image);
	...
	if(type == UAV_VERTICAL)
	{
		for(int i = start.x; i < end.x; i += stepX)
		{
			temp.clear();
			for(int j = start.y; j < end.y; j += stepY)
			{
				temp.push_back(j*image.cols + i);
			}
...
ScanLines::ScanLines (cv::Mat &image_, unsigned scanType, cv::Point center_,
